...
\begin{enumerate}
	\item \textbf{Packet Blocking Functionality:} Incorporation of an optional firewall layer that could enable to block the connections to a certain IP endpoint when the outbound IP address is flagged as malicious. This could be implemented using the Android's VpnService \texttt{protect()} API to drop the packets or connections if some particular conditions are met.
	
	\item \textbf{Enhanced Performance Benchmarking:} Future assessments of Threat Detector can be done using Android's built-in benchmark library (\texttt{androidx.benchmark}\footnote{\url{https://developer.android.com/jetpack/androidx/releases/benchmark}}) to measure memory allocation, CPU time and consumption, and also battery usage more accurately. Additionally, some other profiling tools such as \emph{Systrace}\footnote{\url{https://source.android.com/docs/core/tests/debug/systrace}}, \emph{Perfetto}\footnote{\url{https://perfetto.dev/}}, and \emph{Battery Historian}\footnote{\url{https://developer.android.com/topic/performance/power/battery-historian}}.
	
	\item \textbf{Historical Data and Visualization:} Implementing a persistent database (e.g. Room) to manage and store more modifiable preferences and keep track of the applications that consistently connect to potentially high-risk IP addresses.
	
	\item \textbf{Automated Testing Frameworks:} Integration with \emph{Espresso}\footnote{\url{https://developer.android.com/training/testing/espresso/}} and \emph{JUnit}\footnote{\url{https://developer.android.com/training/testing/local-tests}} could potentially automate user flows for a more systematic and large-scale testing.
	...
	\item \textbf{Privacy and GDPR\footnote{\url{https://gdpr-info.eu/}} Considerations:} Ensuring the anonymization of the IP lookups and further features to comply with data protection standards.
	
	\item \textbf{Integration of Additional Intelligence Sources:} Expanding the threat intelligence sources by incorporating the API from various providers such as \emph{VirusTotal}\footnote{\url{https://blog.virustotal.com/2023/02/monitoring-infrastructure.html}}, \emph{AlienVault OTX}\footnote{\url{https://otx.alienvault.com/}}, and \emph{GreyNoise}\footnote{\url{https://www.greynoise.io/}}. This decreases the reliance on a single threat intelligence source.	
\end{enumerate}
