...
\begin{enumerate}
	...
	\item \textbf{Presentation layer} graphically depicting the results and configuration options to the user/administrator.
\end{enumerate}
...
Threat Detector targets Android 12 (API 31) and above to ensure compatibility with the majority of Android devices in use and alignment with Android's new permission and network models.
...
One of the core functionalities of Threat Detector is its TCP server running locally on the Android device. It listens to the incoming connections on a specified port (default set to 1234 - an arbitrary port that does not interfere with critical background services in Android systems) and handles each connection request in a separate thread.
...
In \cref{fig:Full_Packet_Dump} the extracted information including the destination IP addresses are added to a queue for further analysis and querying from AbuseIPDB.
...
The application page that uses \texttt{SharePreferences} in Threat Detector is shown in \cref{fig:Threat_Detector_Settings_Page}.
...
\begin{itemize}
	\item \textbf{Monitor Page - \cref{fig:Monitor_Page.jpg}:} provides the functionality to start/stop the local TCP server alongside depicting the server's activation status and user-defined IP confidence score.
	\item \textbf{Logs Page - \cref{fig:Logs_Page}:} provides the user with a scrollable list of the applications that attempted to communicate with external IP addresses. Each list Entry shows the application name and icon along with the IP address and its confidence score.
...
The user interface components update dynamically as new incoming data is registered by the TCP server. This includes the real time automatic updates to modify the items shown in the Logs page, the colour change indicating the server status in Monitor page, and the illustration of the	 IP Exhaustion Page as a pop-up when the user runs out of free API calls.
...
\begin{itemize}
	\item Secure storage of the API key in the private application folder.
